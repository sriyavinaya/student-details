...
const ProfileVerification = () => {
  ...
  const handleApprove = (id: string) => {
    setProfiles(prevProfiles => 
      prevProfiles.map(profile => 
        profile.id === id ? {...profile, status: "Approved" as const} : profile
      )
    );
    toast({
  ...
  const submitComment = () => {
    ...
    if (selectedProfile.status === "Pending") {
      setProfiles(prevProfiles => 
        prevProfiles.map(profile => 
          profile.id === selectedProfile.id ? {...profile, status: "Rejected" as const} : profile
        )
      );
      toast({
  ...
  const viewProfileDetails = (profile: StudentProfile) => {
