...
const EditFields = () => {
  ...
  const availableFields = FIELD_TYPES[selectedCategory as keyof typeof FIELD_TYPES];
  
  // Get the options for the selected field
  const fieldOptions = options[selectedCategory as keyof typeof options]?.[selectedField as any] || [];

  const handleCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const category = e.target.value;
    setSelectedCategory(category);
    // Set default field for the new category
    setSelectedField(FIELD_TYPES[category as keyof typeof FIELD_TYPES][0]);
  };
  ...
  const handleAddOption = () => {
    ...
    setOptions(prev => {
      const updated = { ...prev };
      updated[selectedCategory as keyof typeof options][selectedField as any] = [
        ...fieldOptions,
        newOption.trim()
      ];
      return updated;
