...
const TechnicalEventsForm = ({ event, onClose, onSave, refreshTable }) => {
    ...
    useEffect(() => {
        const fetchDropdownOptions = async () => {
            setIsLoading(true);
            try {
                // Fetch category options (technical category)
                const categoryResponse = await fetch(
                    `http://localhost:8080/api/dropdown/fetch?category=technical&dropdownName=Category`
                );
                const categoryData = await categoryResponse.json();
                
                // Fetch achievement options (technical category)
                const achievementResponse = await fetch(
                    `http://localhost:8080/api/dropdown/fetch?category=technical&dropdownName=Achievement`
                );
    ...
    const validateDate = (dateString) => {
        ...
        // This catches cases like "2023-04-31" which gets silently converted to "2023-05-01"
