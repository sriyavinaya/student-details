...
const SportsEventsTable = ({ events = [] }) => {
  ...
  const handleDownload = async (eventId) => {
    try {
      ...
      link.href = url;
      link.download = `sports_event_${eventId}`;
          ...
          {paginatedEvents.length > 0 ? (
            paginatedEvents.map((event) => (
              <TableRow key={event.id}>
                ...
                <TableCell>{event.eventLevel}</TableCell>
                <TableCell>{event.role}</TableCell>
                <TableCell>{event.outcome}</TableCell>
                <TableCell>
                  <span className={`text-sm font-semibold ${
                    event.verificationStatus === "Pending" ? "text-yellow-500" :
                    event.verificationStatus === "Approved" ? "text-green-600" : "text-red-600"
                  }`}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-medium text-gray-800">Sports Event Details</h3>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-100 p-4 rounded-lg">
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-500">EVENT LEVEL</h4>
                  <p className="font-medium text-gray-900">{selectedEvent.eventLevel}</p>
                </div>
                <div>
              ...
              <div className="space-y-4 bg-white p-4 rounded-lg border border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{selectedEvent.role}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">OUTCOME</h4>
                    <p className="font-medium text-gray-900">{selectedEvent.outcome}</p>
                  </div>
                </div>
